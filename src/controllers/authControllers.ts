...
export const signUp = async (req : Request, res : Response) => {
    try {
        const username: User /* si es find({}) porner User[] */
        | null = await userModel.findOne({username: req.body.username});
        if (username) 
...
export const signIn = async (req : Request, res : Response) => {
    try {
        ...
        const comparedPassword = await userModel.desencrypt(req.body.password, user !.password);
        if (! comparedPassword) 
